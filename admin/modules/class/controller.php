...
function doupdatetime(){
	...
	if ($_POST['subjcode'] == "" OR $_POST['subjdesc'] == "" OR $_POST['unit'] == "") {
		...
		$ay 			= $_POST['sy'];
			...
			message($subjcode. " Has sido actualizado exitosamente!", "info");
			redirect('index.php');
...
function doInsert(){
	...
	if ($_POST['coursename'] == "" OR $_POST['coursedesc'] == "") {
		...
		if ($res >=1) {
			message("Nombre del curso ya existes!","error");
			check_message();
		}else{
			...
			$course->DEPT_ID = $coursedept;	
			 ...
			 if ($istrue == 1){
			 	...
			 	message("Nuevo [". $coursename ."] curso creado exitosamente!","Exitoso");
			 	redirect('index.php');
			 }
		}	 
...
function doEdit(){
	...
	if (isset($_POST['savecourse'])){
		...
		if ($_POST['coursename'] == "" OR $_POST['coursedesc'] == "") {
			...
			message($coursename. " has sido actualizado exitosamente!", "info");
			redirect('index.php');
...
function doDelete(){
	...
	message("Curso(s) eliminafo!","info");
	redirect('index.php');
}
?>