		...
		//multi delete using checkbox as a selector
			...
			for($i=0;$i<$key;$i++){
				...
				$intructorClass->delete($id[$i]);
					...
					message("Asignaturas eliminadas!","info");
					redirect('index.php?view=instSubj&instructorId='.$INST_ID.'');
		}else{
			message("Selecciona tu asignatura primero, si deseas eliminarla!","error");
			redirect('index.php?view=instSubj&instructorId='.$INST_ID.'');
		}
}
...
for ($i=0; $i<$subjId; $i++){
	...
	message("La facultad ya esta añadida!","info");
	redirect('index.php?view=instSubj&instructorId='.$instructorId.'');
} 
}else{
	message("!Seleccione primero la asignatura que desea agregar","error");
...
if (isset($_POST['savefaculty'])){
	...
	if ($_POST['name'] == "" OR $_POST['address'] == "" OR $_POST['email'] == "") {
		...
		if ($resuser >=1) {
			message("Nombre de cuenta ya existente!", "error");
			redirect('index.php');
		}else{
		...
		$res = $inst->find_all_instructor($name);
			...
			if ($res >=1) {
				message("El nombre del maestro ya existe!","error");
				check_message();
			}else{
			...
			$istrueee = $inst->create(); 
			 if ($istrueee == 1){
			 	...
			 	message("Nuevo maestro creado exitosamente!","exitoso");
			 	redirect('index.php');
			 }else{

				message("Maestro no creado!","error");
			 	redirect('index.php');
	...
	if ($_POST['name'] == "" OR $_POST['address'] == "" OR $_POST['email'] == "") {
		...
		$empStats 		= $_POST['empStats'];	
			...
			$inst->update($instid);
			 	...
			 	message($name."Ha sido actualizado con exito!","success");
			 	redirect('index.php');
		}	 
...
function doDelete(){
	...
	for($i=0;$i<$key;$i++){
		...
		$inst->delete($id[$i]);
			...
			message("Nombre de facultad ya eliminado!","info");
			redirect('index.php');
}
